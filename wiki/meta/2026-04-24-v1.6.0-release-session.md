...
- wiki-ingest wording about immutable `_raw/` versus maintained manifest
...
- `scripts/allocate-address.sh` and the `address: c-NNNNNN` convention for Mechanism 2. The allocator is flock-guarded and supports re-ingest idempotency through `_raw/.manifest.json address_map`.
...
- `hooks/hooks.json` stages `.vault-meta/` in addition to `wiki/` and `_raw/`, so DragonScale runtime state is captured by the auto-commit hook
...
- `skills/wiki-ingest/SKILL.md`: clarified the difference between immutable user-dropped source documents in `_raw/` and the maintained `_raw/.manifest.json`
