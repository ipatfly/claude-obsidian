...
If the vault has adopted DragonScale Mechanism 2 (detected by `[ -x ./_system/scripts/allocate-address.sh ] && [ -d ./.vault-meta ]`):

- **Parallel ingest sub-agents MUST NOT call `_system/scripts/allocate-address.sh` directly.** The allocator is flock-guarded for atomicity, but the `_raw/.manifest.json` `address_map` update pattern assumes single-writer semantics.
...
- Call `_system/scripts/allocate-address.sh` from inside a parallel sub-agent (single-writer rule above)
