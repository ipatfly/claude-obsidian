...
Before ingesting any file, check `_raw/.manifest.json` to avoid re-processing unchanged sources.

```bash
# Check if manifest exists
[ -f _raw/.manifest.json ] && echo "exists" || echo "no manifest yet"
```
...
{
  "sources": {
    "_raw/articles/article-slug-2026-04-08.md": {
      "hash": "abc123",
...
4. **Save** to `_raw/articles/[slug]-[YYYY-MM-DD].md` with a frontmatter header:
   ```markdown
   ---
   source_url: [url]
   fetched: [YYYY-MM-DD]
   ---
   ```
5. Proceed with **Single Source Ingest** starting at step 2 (file is now in `_raw/`).
...
3. **Save** the description to `_raw/images/[slug]-[YYYY-MM-DD].md`:
   ```markdown
   ---
   source_type: image
...
**CRITICAL**: never use the Write or Edit tool on `.vault-meta/address-counter.txt`. That would fire the PostToolUse hook, which runs `git add wiki/ _raw/` and can accidentally commit unrelated pending wiki changes under a generic message. Counter mutation is **only** permitted through the helper script (Bash tool).
...
### `address_map` in `_raw/.manifest.json`
