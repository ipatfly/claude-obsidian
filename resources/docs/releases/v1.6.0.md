...
- Gated on: `bash _system/bin/setup-dragonscale.sh`, plus local dependencies per mechanism.
...
Those fixes covered tiling report path confinement, rollout baseline date, AGENTS wording, `_raw/` manifest wording, and install-guide version drift.

`1.6.0` was Phase 4.
It shipped Mechanism 4: boundary-first autoresearch.
It added `_system/scripts/boundary-score.py`, wired no-topic `/autoresearch` into frontier suggestions, added boundary-score tests, and bumped the DragonScale spec to v0.4.
...
Invoke:
run `./_system/scripts/allocate-address.sh` directly, or let `wiki-ingest` assign addresses after DragonScale setup.
...
Invoke:
run `python3 ./_system/scripts/tiling-check.py --peek` for readiness, or `python3 ./_system/scripts/tiling-check.py --report wiki/meta/tiling-report-YYYY-MM-DD.md` for a saved report.
...
Invoke:
run `/autoresearch` with no topic in a DragonScale-enabled vault, or run `python3 ./_system/scripts/boundary-score.py --json --top 5`.
